...
// update referrals data when user register
...
async function updRefIncome(data,copunWorth){
    const user = User.findOne({referralCode:data.referredBy})
    const userIdToFind = data._id; // Replace with the actual user ID you want to find

    const userRerral =await User.find(
        {
            'referrals._id': userIdToFind
        }
    );
    userRerral.forEach((i)=>{
        const refIndex = i.findIndex(referral=>referral._id==userIdToFind)
        if(i.referrals[refIndex].level==1){ 
            i.earning = i.earning + copunWorth*10/100 || 0}
        else if(i.referrals[refIndex].level==2){ i.earning = i.earning + copunWorth*5/100 || 0}
        else if(i.referrals[refIndex].level==3){  i.earning = i.earning + copunWorth*3/100 || 0}
        else if(i.referrals[refIndex].level==4){  i.earning = i.earning + copunWorth*2/100 || 0}
        else if(i.referrals[refIndex].level==5){  i.earning = i.earning + copunWorth*1/100 || 0}

    })

    // const userRerral = user.
    if(user){
        // user.earning=user.earning+copunWorth*

        if(user.level==1){ totalIncome= totalIncome + users.coupons.reduce((accumulator, coupon) => accumulator + (coupon.amount || 0), 0)*10/100 || 0}
        else if(user.level==2){ totalIncome= totalIncome + users.coupons.reduce((accumulator, coupon) => accumulator + (coupon.amount || 0), 0)*5/100 || 0}
        else if(user.level==3){ totalIncome= totalIncome + users.coupons.reduce((accumulator, coupon) => accumulator + (coupon.amount || 0), 0)*3/100 || 0}
        else if(user.level==4){ totalIncome= totalIncome + users.coupons.reduce((accumulator, coupon) => accumulator + (coupon.amount || 0), 0)*2/100 || 0}
        else if(user.level==5){ totalIncome= totalIncome + users.coupons.reduce((accumulator, coupon) => accumulator + (coupon.amount || 0), 0)*1/100 || 0}
    }
}
